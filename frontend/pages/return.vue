      ...
      <main class="flex-1 p-6">
        <div class="bg-white shadow-lg rounded-lg p-6 max-w-7xl mx-auto">
          ...
          <div class="mb-6 flex items-center justify-between">
            <div class="flex mt-3">
              <h2
                @click="setStatusFilter('borrowed')"
                :class="[
                  'cp me-5',
            ...
            <table class="min-w-full divide-y divide-gray-200 bg-white rounded-lg shadow">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(list, i) in filteredLists" :key="list.id">
                  ...
                  <td
                    v-if="list.status === 'returned'"
                    class="px-4 py-2 text-sm text-gray-900"
                  >
                    {{ formatDate(list.returnedDate) }}
                  </td>
                  <td class="px-4 py-2 text-sm text-gray-900">{{ list.fine }}</td>
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: e.data?.message || e.message || 'Delete failed'
        })
      }
    }
  }
  ...
  async function confirmReturn(list) {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: e.data?.message || e.message || 'Return failed'
      })
    }
  }
