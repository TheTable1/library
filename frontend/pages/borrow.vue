  ...
  <div class="flex min-h-screen bg-gray-50">
    ...
    <main class="flex-1 p-6">
      <div class="bg-white shadow-lg rounded-lg p-6 max-w-4xl mx-auto">
        <!-- Header -->
        <div class="mb-6">
          <h1 class="text-2xl font-bold text-gray-800">Create Loan</h1>
          <p class="text-gray-600 mt-1">Search, select books, and assign to a user</p>
        </div>
        ...
        <form @submit.prevent="onSubmit" class="space-y-6">
          ...
          <div class="flex justify-end">
            <button
              type="submit"
              :disabled="loading"
              class="flex items-center gap-2 bg-darkblue text-white font-semibold px-6 py-2 rounded-lg transition disabled:opacity-50"
            >
              <UIcon name="i-lucide-book-open" class="text-lg" />
              <span v-if="!loading">Borrow</span>
            </button>
          </div>
        </form>
      </div>
    ...
    >
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 text-center space-y-4">
        ...
        <h2 class="text-xl font-semibold text-gray-800">Loan created successfully!</h2>
        <button
          @click="closeSuccess"
...
async function fetchBooks() {
  ...
  try {
    ...
    books.value = res.data.filter(b => b.availableCopies > 0)
  } catch (e) {
...
async function onSubmit() {
  ...
  try {
    ...
    error.value = e.data?.message || e.message || 'Loan failed'
  } finally {
    loading.value = false
  }
}
